...
export function FloatingAdminPanel({
  ...
  // 📦 LOCAL ACCORDION COLLAPSE STATES
  ...
  return (
    <div
      ...
      className={`absolute pointer-events-auto bg-neutral-900/95 border border-neutral-800 backdrop-blur-md rounded-2xl shadow-2xl flex flex-col overflow-hidden select-none z-40 max-w-[calc(100vw-32px)] max-h-[calc(100vh-90px)] landscape:w-72 landscape:h-[calc(100vh-100px)] ${cleanViewActive ? 'w-auto h-auto rounded-xl border-cyan-500/30' : ''}`}
    >
      {/* HEADER DRAG HANDLE */}
        ...
        {!cleanViewActive ? (
          <>
            ...
            <div className="flex items-center gap-2">
              {/* ✅ MOVE UP: The view toggler is safely fixed inside the header bar */}
              <button
                type="button"
          ...
          <button
            type="button"
            onClick={onToggleCleanView}
            className="w-full bg-cyan-500 hover:bg-cyan-400 text-neutral-950 font-black text-[10px] uppercase tracking-wider py-1 rounded-md transition-colors"
          >
            👁️ Open Studio Controls
          </button>
        )}
      </div>

      {/* SCROLL CONTAINER (HIDDEN IF CLEAN VIEW IS ENGAGED) */}
      <div className={`flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar ${cleanViewActive ? 'hidden' : ''}`}>

        {/* MASTER GATEWAY PANEL */}
        ...
        {/* 🎨 ACCORDION CARD: SURFACE PAINTER */}
        <div className="bg-neutral-950 rounded-xl border border-neutral-800/80 overflow-hidden transition-all">
          <button
            type="button"
            onClick={() => setIsPainterOpen(!isPainterOpen)}
            className="w-full flex justify-between items-center bg-neutral-950/90 px-3 py-2.5 text-[9px] font-black text-cyan-400 uppercase tracking-wider border-b border-neutral-900"
          >
        ...
        {/* 📱 ACCORDION CARD: CAMERA NAVIGATION OVERRIDE */}
        <div className="bg-neutral-950 rounded-xl border border-neutral-800/80 overflow-hidden transition-all">
          <button
            type="button"
        ...
        {/* 💡 ACCORDION CARD: LIGHT EMISSION MOTORS */}
        <div className="bg-neutral-950 rounded-xl border border-neutral-800/80 overflow-hidden transition-all">
          <button
            type="button"
      ...
      {/* FOOTER BAR */}
      <div className={`bg-neutral-950/40 p-2 border-t border-neutral-800/60 flex items-center justify-between shrink-0 ${cleanViewActive ? 'hidden' : ''}`}>
